...
// Earnings insights — derived from mock job data
export const getEarningsInsights = (jobs = [], currentUser) => {
    const insights = [];
    const completed = jobs.filter(j =>
        (j.providerId === currentUser?.uid || j.provider_id === currentUser?.uid) &&
        ['Completed', 'Paid', 'payment_released'].includes(j.status)
    );

    const weekendJobs = completed.filter(j => {
        const d = j.completedAt ? new Date(j.completedAt) : null;
        return d && (d.getDay() === 0 || d.getDay() === 6);
    });

    if (weekendJobs.length > 0 && completed.length > 0) {
        insights.push({
            icon: 'weekend',
            text: `You complete ${Math.round((weekendJobs.length / completed.length) * 100)}% of your jobs on weekends — providers who stay active all week earn up to 2× more.`,
        });
    }
    ...
    if (thisMonth.length >= 0) {
        ...
        insights.push({
            icon: 'trending_up',
            text: thisMonth.length >= target
                ? `Outstanding! You've completed ${thisMonth.length} jobs this month — you're in the top tier of providers on TaskMate.`
                : `You've completed ${thisMonth.length} job${thisMonth.length !== 1 ? 's' : ''} this month. Top providers hit ${target}+ — keep your availability on to get more requests.`,
        });
    }
    ...
    if (category && PRICE_RANGES[category]) {
        ...
        insights.push({
            icon: 'price_check',
            text: `Market rate for ${category} in Nigeria: ₦${range.min.toLocaleString()} – ₦${range.max.toLocaleString()}. Aim for ₦${range.avg.toLocaleString()} to stay competitive and profitable.`,
        });
    }
    ...
    if (insights.length === 0) {
        insights.push({
            icon: 'lightbulb',
            text: 'Complete your first job to unlock personalised earnings insights and performance analytics.',
        });
    }

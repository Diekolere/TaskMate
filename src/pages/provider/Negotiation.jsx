...
const Negotiation = () => {
    ...
    useEffect(() => {
        ...
        if (foundJob) {
            setJob(foundJob);
            setLoading(false);
            // If job is provider_accepted, start negotiation
            if (foundJob.status === 'provider_accepted' && isCustomer) {
                startNegotiation(id);
            }
        } else {
            setTimeout(() => {
                ...
                if (retry) {
                    setJob(retry);
                    if (retry.status === 'provider_accepted' && isCustomer) {
                        startNegotiation(id);
                    }
                } else {
    ...
    }, [id, requests, jobs, isCustomer, startNegotiation, navigate]);
    ...
    useEffect(() => {
        ...
        if (filtered.length > 0) {
            setMessages(filtered.map(m => ({
                id: m.id,
                sender: m.sender_id === currentUser.id ? 'provider' : 'customer',
                message: m.message,
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2">
                                <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
                                    {/* Messages */}
                                    <div className="h-[50vh] min-h-[300px] max-h-[440px] overflow-y-auto space-y-4 mb-6">
                                        <AnimatePresence>
                                            {messages.map((msg) => (
                                                <motion.div
                                                    key={msg.id}
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3">
                                            ...
                                            <button
                                                onClick={sendMessage}
                                                className="w-full sm:w-auto px-6 py-3 bg-[#0F172A] hover:bg-slate-700 text-white text-sm font-bold rounded-xl transition-all"
                                            >
                                                Send
                                            </button>
                                        </div>

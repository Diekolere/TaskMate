...
const ChangePassword = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setTimeout(() => navigate('/provider/settings'), 2000);
        } catch (error) {
                            ...
                            <div className="p-8">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    {[
                                        ...
                                        { name: 'newPassword', label: 'New Password', placeholder: 'Enter new password', hint: 'Minimum 8 characters' },
                                        { name: 'confirmPassword', label: 'Confirm New Password', placeholder: 'Confirm new password' },
                                    ].map(({ name, label, placeholder, hint }) => (
                                        <div key={name} className="space-y-2">
                                            <label className="block text-[10px] font-black uppercase tracking-widest text-charcoal/40 ml-1">{label}</label>
                                            <input
                                                type="password"
                                                name={name}
                                                value={formData[name]}
                                                ...
                                                required
                                            />
                                            {hint && <p className="text-[10px] font-black uppercase tracking-widest text-charcoal/30 ml-1">{hint}</p>}
                                        </div>
                                    ))}

                                    <div className="pt-6 flex items-center gap-4 border-t border-charcoal/5 mt-8">
                                        <button type="button" onClick={() => navigate('/provider/settings')} className="px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-charcoal/40 hover:bg-charcoal/5 transition-colors">
                                            Cancel
                                        </button>
                                        <button type="submit" disabled={isLoading} className="flex-1 btn-lime justify-center rounded-2xl py-4">
                                            {isLoading ? (
                                                <>

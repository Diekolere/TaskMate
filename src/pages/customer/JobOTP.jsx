...
const JobOTP = () => {
    ...
    useEffect(() => {
        ...
        const tick = setInterval(() => {
            setSecondsLeft(s => {
                if (s <= 1) { setStatus('expired'); clearInterval(tick); return 0; }
                return s - 1;
            });
            setElapsed(e => e + 1);
        }, 1000);
    ...
    useEffect(() => {
        if (status === 'started') {
            setTimeout(() => navigate(`/customer/confirm/${jobId}`), 3000);
        }
                        ...
                        <AnimatePresence mode="wait">
                            {status === 'expired' && (
                                ...
                                >
                                    ...
                                    <h2 className="text-xl font-bold text-gray-900 mb-2">Code Expired</h2>
                                    <p className="text-sm text-gray-400 max-w-xs mb-8">The 15-minute window has passed. Generate a new code by refreshing this page, or contact your provider.</p>
                                    <button
                                        onClick={() => { setSecondsLeft(OTP_EXPIRY_SECONDS); setElapsed(0); setStatus('waiting'); }}
                            ...
                            {status === 'waiting' && (
                                ...
                                >
                                    ...
                                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 w-full flex items-center gap-3">
                                        <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 ${secondsLeft < 120 ? 'bg-red-50' : 'bg-amber-50'}`}>
                                            <span className={`material-icons text-lg ${secondsLeft < 120 ? 'text-red-500' : 'text-amber-500'}`}>schedule</span>
                                        </div>
                                        <div>
                                        ...
                                        <div className="ml-auto text-right shrink-0">
                                            ...
                                            <p className={`text-sm font-mono font-bold ${secondsLeft < 120 ? 'text-red-500' : 'text-gray-700'}`}>
                                                {pad(minsLeft)}:{pad(secsLeft)}
                                            </p>
                                        </div>
                                    </div>
